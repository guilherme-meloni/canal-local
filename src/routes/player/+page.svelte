<script lang="ts">
	...
	function connectToEmissora() {
		...
		isLoading = true;
		connectionStatus = `Conectando a ${WS_URL.replace('ws://', '')}...`;
		...
		socket.onclose = () => {
			...
			if (reconnectAttempts < maxReconnectAttempts) {
				...
				setTimeout(connectToEmissora, delay);
			} else {
				connectionStatus = 'Falha ao conectar. Verifique o IP e recarregue a página.';
			}
		};
		socket.onerror = () => {
